...
useEffect(()=>{ 
  ...
  function editUserData(id) {
    axios.get(`http://localhost:8000/api/user/data/${id}`).then(getUserData=>JSON.stringify(getUserData.data.data)).then(json=>{
      const data = JSON.parse(json);
      console.log(data);
      setUserData(data);
  ...
  const column = [
    ...
    },
    {
      ...
      selector:function ({user_id}) {
        return <>
        <i className="bi bi-pencil-square" onClick={() => editUserData(user_id)} style={{fontSize:"20px",color:"blue",margin:"8px"}} data-bs-toggle="modal" data-bs-target="#myEdit"></i>
        <i className="bi bi-eye" onClick={() => viewUserData(user_id)} style={{fontSize:"20px",color:"green",margin:"8px"}} ></i>
  ...
  <div className="modal-dialog">
    <div className="modal-content">
      <div className="modal-header">
        ...
        <button type="button" className="btn-close" onClick={closePopUpView} data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div className="modal-body">
        <div className="row">
